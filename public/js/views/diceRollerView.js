// public/js/views/diceRollerView.js
//
// A simple dice roller available to both the DM and players (unlike the DM
// panel / Monsters sidebar, this widget is never hidden by role). Rolls are
// computed server-side (see server/socketHandlers/diceHandler.js) and
// broadcast to everyone — a shared, public roll log, same as rolling dice
// on a physical table.
//
// The widget is draggable (by its header) and collapsible, since it floats
// on top of the board/panels and different tables will want it tucked in a
// different corner. Position + collapsed state persist in localStorage so
...
// ---- Drag (by the header) ----
//
// The panel is positioned with `right`/`bottom` by default (see style.css);
// once dragged we switch to explicit `left`/`top` pixel coordinates so it
// can be placed anywhere, and remember that in localStorage.
