...
// DM-only board tools (overlay placement).
//
// Like overlayPanelView.js, this attaches its own listeners to the board
// element and only acts while armed (`boardTool.type === 'measure'`) rather
// than boardView.js knowing anything about measuring.

import { board, boardTool, setBoardTool } from "../state.js";
...
measureToggleBtn.addEventListener("click", () => {
  const active = boardTool.type === "measure";
  if (active) {
    disarm();
  } else {
    setBoardTool({ type: "measure" });
...
boardEl.addEventListener("pointerdown", (e) => {
  if (boardTool.type !== "measure") return;
  measuring = true;
...
boardEl.addEventListener("pointermove", (e) => {
  if (!measuring) return;
  drawLine(startCell, cellFromEvent(e));
});
...
function drawLine(start, current) {
  ...
  measureLayer.setAttribute("viewBox", `0 0 ${board.grid.cols * cs} ${board.grid.rows * cs}`);
