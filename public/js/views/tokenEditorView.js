...
import { dmRoster } from "../state.js";

import { openCharacterModal } from "./characterModalView.js";
import { renderTokenList } from "./dmPanelView.js";

// Which token's editor is currently expanded (module-level UI state — only
// one at a time, and only meaningful for the DM, who's the only one that
// ever renders this list).
...
export function buildTokenListItem(token) {
  ...
  editBtn.addEventListener("click", () => {
    expandedTokenId = isExpanded ? null : token.id;
    renderTokenList();
  });
  ...
  if (isExpanded) li.appendChild(buildEditor(token));

  return li;
}

function buildEditor(token) {
  ...
  if (token.owner) {
    const rosterEntry = dmRoster.find((r) => r.username === token.owner);
    if (rosterEntry) {
      editor.appendChild(buildLinkedCharacterSummary(rosterEntry.username, rosterEntry.character));
    } else {
...
function buildLinkedCharacterSummary(username, character) {
