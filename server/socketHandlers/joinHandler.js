...
import { addActivePlayer, removeActivePlayer, broadcastOnlinePlayers, pushAllCharactersToDMs } from '../rosterStore.js';
...
export function registerJoinHandler(io, socket, session) {
  socket.on(EVENTS.JOIN, ({ mode, name, characterId }) => {
    ...
    pushAllCharactersToDMs(io); // refresh every connected DM's roster — covers first joins AND reconnects
